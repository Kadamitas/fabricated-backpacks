...
final class StorageClientAcceptance {
        ...
        for (boolean releaseOutside : new boolean[]{false, true}) {
            ...
            try {
                ...
                if (releaseOutside) context.runOnClient(client -> {
                    ...
                    check(origin.getLeftPos() > 1 && origin.getTopPos() > 1,
    ...
    private static double[] storageSlotPosition(ClientGameTestContext context, int index) {
        return context.computeOnClient(client -> {
            ...
            return new double[]{(origin.getLeftPos() + slot.x + 8.0) * client.getWindow().getScreenWidth() / client.getWindow().getGuiScaledWidth(),
                    (origin.getTopPos() + slot.y + 8.0) * client.getWindow().getScreenHeight() / client.getWindow().getGuiScaledHeight()};
        });
    }
    ...
    private static double[][] dragSlotPositions(ClientGameTestContext context) {
        return context.computeOnClient(client -> {
            ...
            for (int index = 0; index < points.length; index++) {
                ...
                int x = origin.getLeftPos() + slot.x;
                int y = origin.getTopPos() + slot.y;
