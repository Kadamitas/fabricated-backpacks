...
public final class PiglinSafetyGameTests {
    ...
    public static void goldBackpackRules(GameTestHelper helper) {
        ...
        helper.assertFalse(PiglinAi.isWearingGold(player), "Holding or carrying a gold backpack does not count as wearing it");
        for (BackpackTier tier : BackpackTier.values()) {
            BackpackEquipment.set(player, new ItemStack(BackpackRegistry.item(tier)));
            helper.assertValueEqual(PiglinAi.isWearingGold(player), tier == BackpackTier.GOLD,
        ...
        helper.assertTrue(PiglinAi.isWearingGold(player), "Ordinary golden armor remains valid alongside a non-gold native backpack");
        player.setItemSlot(EquipmentSlot.HEAD, ItemStack.EMPTY);
        BackpackEquipment.set(player, gold);
        Piglin piglin = helper.spawn(EntityType.PIGLIN, new BlockPos(3, 1, 3));
        ...
        helper.assertFalse(PiglinAi.isWearingGold(player), "Removing native gold equipment removes only its passive protection");
        helper.succeed();
    }
}
