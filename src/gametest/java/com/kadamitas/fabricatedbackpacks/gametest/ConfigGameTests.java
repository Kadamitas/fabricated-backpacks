...
public final class ConfigGameTests {
    ...
    public static void configuredJukeboxResize(GameTestHelper helper) {
        ...
        try {
            ...
            helper.assertValueEqual(NbtAccess.getIntOr(bag.settings(upgrade), "active_slot", -1), 5, "The doubled twenty-four-slot default has active audio and history");
        ...
        helper.runAfterDelay(2, () -> {
            ...
            try {
                ...
                JukeboxRuntime.action(bag, upgrade, helper.getLevel(), position, null, "next");
                helper.assertValueEqual(NbtAccess.getIntOr(bag.settings(upgrade), "active_slot", -1), 198, "Expanded queue includes actual new high-index records");
                JukeboxRuntime.action(bag, upgrade, helper.getLevel(), position, null, "next");
                helper.assertValueEqual(NbtAccess.getIntOr(bag.settings(upgrade), "active_slot", -1), 199, "Playback reaches the final physical slot in a 200-record library");
                JukeboxRuntime.action(bag, upgrade, helper.getLevel(), position, null, "previous");
                helper.assertValueEqual(NbtAccess.getIntOr(bag.settings(upgrade), "active_slot", -1), 198, "Playback history returns from physical slot 199 to 198");
                JukeboxRuntime.action(bag, upgrade, helper.getLevel(), position, null, "previous");
