...
final class EquipmentGameTests {
    ...
    static void nativeSlotAndDeath(GameTestHelper helper) {
        ...
        boolean keep = level.getGameRules().getBoolean(GameRules.RULE_KEEPINVENTORY);
        try {
            ...
            helper.assertValueEqual(BackpackEquipment.visual(player).get(BagComponents.COLORS),
                    expected.get(BagComponents.COLORS), "Private storage filtering retains the actual wearable colors");
            ...
            keeper.hurt(keeper.damageSources().genericKill(), Float.MAX_VALUE);
            ...
            cursedPlayer.hurt(cursedPlayer.damageSources().genericKill(), Float.MAX_VALUE);
            ...
            level.getGameRules().getRule(GameRules.RULE_KEEPINVENTORY).set( keep, server);
        }
        helper.succeed();
    }
}
