...
public final class BackpackInput {
    ...
    public static void initialize(KeyMapping.Category category) {
        ...
        for (int index = 0; index < toggles.length; index++) toggles[index] = KeyMappingHelper.registerKeyMapping(new KeyMapping(
                "key.fabricated_backpacks.upgrade_" + (index + 1), index == 0 ? com.mojang.blaze3d.platform.InputConstants.KEY_Z : index == 1 ? com.mojang.blaze3d.platform.InputConstants.KEY_X : com.mojang.blaze3d.platform.InputConstants.UNKNOWN.getValue(), category));
