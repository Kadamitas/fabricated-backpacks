...
public final class ChestLoot {
    ...
    private ChestLoot() { }
    /** Resource origins observed while the registry loads; Forge's loot event does not expose them. */
    private record Observed(net.minecraft.resources.ResourceKey<?> key, net.minecraft.server.packs.resources.Resource source) {}
    private static final java.util.Map<net.minecraft.resources.Identifier, Observed> SOURCES = new java.util.concurrent.ConcurrentHashMap<>();
    public static void initialize() {
        net.minecraftforge.event.LootTableLoadEvent.BUS.addListener((net.minecraftforge.event.LootTableLoadEvent event) -> {
            Observed observed = SOURCES.remove(event.getName());
            if (observed != null) addBuiltinPool(observed.key(), event.getTable(), observed.source());
        });
    }
    public static void observeSource(net.minecraft.resources.ResourceKey<?> key, net.minecraft.server.packs.resources.Resource source) {
        if (key.isFor(net.minecraft.core.registries.Registries.LOOT_TABLE)) SOURCES.put(key.identifier(), new Observed(key, source));
    }
