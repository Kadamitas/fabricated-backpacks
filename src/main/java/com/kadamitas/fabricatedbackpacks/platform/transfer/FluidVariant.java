...
public record FluidVariant(Fluid getFluid, DataComponentPatch getComponentsPatch) implements TransferVariant<Fluid> {
    public FluidVariant(FluidResource resource) { this(resource.getFluid(), resource.getComponentsPatch()); }
    // Native FluidResource construction consults bound registry components. A
    // saved identity may be declared during registration; defer that lookup to
    // the runtime capability/rendering boundary instead of initializing it here.
    public FluidResource nativeResource() { return FluidResource.of(getFluid, getComponentsPatch); }
