...
public final class ExtendedMenuType<M extends AbstractContainerMenu, D> extends MenuType<M> {
    ...
    public ExtendedMenuType(Factory<M, D> factory, StreamCodec<? super RegistryFriendlyByteBuf, D> codec) {
        super((IContainerFactory<M>) (id, inventory, buffer) -> factory.create(id, inventory,
                codec.decode(new RegistryFriendlyByteBuf(buffer, inventory.player.registryAccess()))), FeatureFlags.DEFAULT_FLAGS);
        this.codec = codec;
    }
